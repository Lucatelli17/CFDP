...
describe('parcours ENTREPRISE FO', () => {
        beforeEach(() => {
        ...
        it('Parcours Entreprise', () => {
                ...
                getIframeBody().find('[class="v-card__title"]').contains('Alsina Entreprise').click()
                ...
                getIframeBody().contains('Retrouver toutes les informations légales par nom').parent().find('[class="v-select__selections"]').type(ParcoursData.re7FO.parcoursENTREPRISE.nomEntreprise)
                cy.wait(1000)
                ...
                cy.wait(3000)
                // Code NAF
                getIframeBody().find('input[data-cy="42"]').click().type(ParcoursData.re7FO.parcoursENTREPRISE.codeNAF)
                getIframeBody().find('[role="listbox"]').contains('20.51Z').click()
                //Nombre de salariés
                getIframeBody().find('[id="Nombre de salariés"]').clear().type(ParcoursData.re7FO.parcoursENTREPRISE.nbSalaries)
                //Nombre de véhicules terrestres à moteur
                getIframeBody().find('[id="Nombre de véhicules terrestres à moteur"]').clear().type(ParcoursData.re7FO.parcoursENTREPRISE.nbVTM)
                // Chiffres d'affaires => ^ = commence par ...
                getIframeBody().find('[id^="Chiffre"]').clear().type(ParcoursData.re7FO.parcoursENTREPRISE.CA)
                // CALCULER
                ...
                cy.wait(5000)
                // Sélection Civilité      
                getIframeBody().find('[class="col col-4"]').find('[role="button"]').type(ParcoursData.re7FO.parcoursENTREPRISE.civilite)
                // Nom & prénom représentant         
                getIframeBody().find('[id="nomRepresentant"]').type(ParcoursData.re7FO.parcoursENTREPRISE.nom)
                getIframeBody().find('[id="prenom"]').type(ParcoursData.re7FO.parcoursENTREPRISE.prenom)
                // En qualité de
                getIframeBody().find('input[data-cy="qualiteProfessionnelle"]').type(ParcoursData.re7FO.parcoursENTREPRISE.qualiteProfessionnelle)
                //procédures judiciaires
                getIframeBody().find('[id="nombreProcedures"]').click().type(ParcoursData.re7FO.parcoursENTREPRISE.nbProcedures)
                // Activité précise
                getIframeBody().find('input[data-cy="activite"]').type(ParcoursData.re7FO.parcoursENTREPRISE.activite)
                ...
                // Redacteur devis
                getIframeBody().find('input[id="emisPar"]').type(ParcoursData.re7FO.parcoursENTREPRISE.emisPar)
                ...
                getIframeBody().find('button').contains('Valider').click()
                cy.wait(2000)
                getIframeBody().find('input[data-cy="telephone1"]').type(ParcoursData.re7FO.parcoursENTREPRISE.telephone)
                getIframeBody().find('input[data-cy="mail"]').type(ParcoursData.re7FO.parcoursENTREPRISE.mail)
                getIframeBody().find('button').contains('Étape suivante').click()
                cy.wait(2000)
                getIframeBody().find('input[data-cy="fractionnement"]').click().type(ParcoursData.re7FO.parcoursENTREPRISE.fractionnement, { force: true }).type('{enter}', { force: true })
                getIframeBody().find('input[data-cy="moyenDePaiement"]').click().type(ParcoursData.re7FO.parcoursENTREPRISE.moyenPaiement, { force: true }).type('{enter}', { force: true })
                getIframeBody().find('button').contains('Enregistrer').click()
                cy.wait(2000)
                getIframeBody().find('button').contains('Signer électroniquement').click()
                getIframeBody().find('input[data-cy="prenom"]').type(ParcoursData.re7FO.parcoursENTREPRISE.prenom)
                getIframeBody().find('input[data-cy="nom"]').type(ParcoursData.re7FO.parcoursENTREPRISE.nom)
                getIframeBody().find('input[data-cy="mail"]').type(ParcoursData.re7FO.parcoursENTREPRISE.mail)
                getIframeBody().find('input[data-cy="portable"]').type(ParcoursData.re7FO.parcoursENTREPRISE.telephone)
                cy.wait(2000)
